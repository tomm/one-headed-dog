...
// Cerberus thing
#include "cerberus.h"
static inline uint8_t fake6502_mem_read(fake6502_context* c, uint16_t address)
{
    return cpeek(address);
}
static inline void fake6502_mem_write(fake6502_context* c, uint16_t address, uint8_t val)
{
    cpoke(address, val);
}
...
uint16_t fake6502_mem_read16(fake6502_context* c, uint16_t addr)
{
    ...
    return ((uint16_t)fake6502_mem_read(c, addr) | ((uint16_t)fake6502_mem_read(c, addr + 1) << 8));
}
...
{ // zero-page,X
    ...
    c->emu.ea = ((uint16_t)fake6502_mem_read(c, (uint16_t)c->cpu.pc++) + (uint16_t)c->cpu.x) & 0xFF;
}

FAKE6502_FN_ADDR_MODE(zpy)
{ // zero-page,Y
    // do zero-page wraparound
    c->emu.ea = ((uint16_t)fake6502_mem_read(c, (uint16_t)c->cpu.pc++) + (uint16_t)c->cpu.y) & 0xFF;
}
...
{ // indirect
    ...
    c->emu.ea = (uint16_t)fake6502_mem_read(c, eahelp) | ((uint16_t)fake6502_mem_read(c, eahelp2) << 8);
    c->cpu.pc += 2;
}
#endif
...
{ // (indirect,X)
    ...
    eahelp = (uint16_t)(((uint16_t)fake6502_mem_read(c, c->cpu.pc++) + (uint16_t)c->cpu.x) & 0xFF);
    c->emu.ea = (uint16_t)fake6502_mem_read(c, eahelp & 0x00FF) | ((uint16_t)fake6502_mem_read(c, (eahelp + 1) & 0x00FF) << 8);
}
